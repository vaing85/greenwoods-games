...
# Greenwood Games - Fixed Deploy to DigitalOcean Server
# Server IP: 161.35.92.24

set -e

echo "🚀 Deploying Greenwood Games to DigitalOcean Server (FIXED VERSION)"
...
# Get domain name
read -p "Enter your domain name (e.g., casino.greenwoodgames.site): " DOMAIN
if [ -z "$DOMAIN" ]; then
...
print_info "Fixing web build issues..."
cd web
...
if grep -q '"react-scripts": "^0.0.0"' package.json; then
    print_warning "Fixing react-scripts version..."
    sed -i 's/"react-scripts": "^0.0.0"/"react-scripts": "5.0.1"/g' package.json
...
if [ $? -eq 0 ]; then
    ...
    print_warning "Web build failed, but continuing with deployment..."
fi
...
echo "   2. Test all casino games and features"
echo "   3. Start marketing your casino platform"
